...
class _College4ScreenState extends State<College4Screen> {
  ...
  List<Map<String, dynamic>> get _colleges {
    ...
    if (_activeTab == "Govt") {
      filtered = filtered
          .where((c) =>
              c['category']?.toString().toLowerCase().contains('govt') ==
                  true ||
              c['type']?.toString().toLowerCase().contains('govt') == true)
          .toList();
    } else if (_activeTab == "Autonomous") {
      filtered = filtered
          .where((c) =>
              c['type']?.toString().toLowerCase().contains('auto') == true)
          .toList();
    }
  ...
  Future<void> _loadAdvertisements() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
            'https://master-backend-18ik.onrender.com/api/advertisements?page=collegepage4'),
      );
  ...
  Future<void> _fetchColleges() async {
    ...
    try {
      ...
      if (widget.degreeId != null) {
        degreeColleges =
            degreeColleges.where((c) => c.degreeId == widget.degreeId).toList();
      } else if (widget.degree != 'All Colleges' && widget.degree.isNotEmpty) {
        // Fallback to name filtering
        degreeColleges =
            degreeColleges.where((c) => c.degreeName == widget.degree).toList();
      }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          if (_isLoading) const GlassLoader(message: 'Loading colleges...'),
        ],
      ),
    );
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: Colors.white,
          borderRadius:
              BorderRadius.circular(isDesktop ? 20 : (isTablet ? 20 : 16)),
          boxShadow: [
            BoxShadow(
