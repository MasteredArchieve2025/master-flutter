...
class _Exam2ScreenState extends State<Exam2Screen> {
  ...
  Future<void> _fetchExamTypes() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _errorMessage = 'Failed to load exam types. Status: ${response.statusCode}';
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  void _createTabsFromData() {
    ...
    for (String type in typeList) {
      if (type != 'other') { // Skip 'other' or include it if you want
                                        ...
                                        child: _filteredExams.isEmpty
                                            ? const Center(
                                                child: Padding(
                                                  ...
                                                  child: Text('No exam types available'),
                                                ),
                                              )
                                            : Wrap(
                                                spacing: gap,
